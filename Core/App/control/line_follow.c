...
void line_follow_control(int16_t base)
{
  ...
  /* ==========================================================================
   * ★★★ 2026-09-24 晚【关键修复】：相切点翻转修正必须在这里生效 ★★★
   *
   *  实车现象（用户原话）："发现小车很明显的抓到了那个窗口，问题是不往外拐，
   *                        仍然在一直绕圈"
   *  —— 识别成功、动作没生效。
   *
   *  病根：翻转覆盖原来只写在两个分支里
   ...
   *  而相切点的实际情况是【线一直都在】（队友判据③："相切点一直有线"）、
   *  图案也是【短暂】宽而不是"持续过久" —— 两个分支都不进！
   *  于是 gourd_flip 被置了 GOURD_FLIP_CYCLES 拍，却从来没作用到 e 上
   *  → 车识别到相切点却"不往外拐" → 一直绕同一个圆。
   *
   *  修法：在正常循迹路径的最后、进 PD 之前，再补一次翻转覆盖。
   *  ★放在 on_cross 之外：十字强制直行优先级更高，别被翻转打断。
   *  ★上面那两处保留（丢线/宽图案时同样需要翻转，删了会退化）。
   * ========================================================================== */
#if USE_GOURD_SM
#if GOURD_USE_FLIP
