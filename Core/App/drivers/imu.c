...
void imu_init(void)
{
  ...
  /* ★GYRO_CONFIG: 0x18 = FS_SEL=3 = **±2000 dps**
     ★★★ 2026-09-26 16:2x 从 ±1000（0x10）改到 ±2000（0x18）★★★
     为什么必须改：实车日志（FW:0926-1800，457 个样本）里 **|GZ| >= 990 出现 34 次（7.4%）**，
     其中 `GZ=-995` 出现 **25 次**、`GZ=+995` 出现 9 次 —— **反复落在同一个数 = 削顶的铁证**
     （真实测量不会反复等于同一个值）。而 995 ≈ 32767/32.8 正好是 ±1000 档的满量程。
     ⇒ 车进葫芦圈后实际转速【超过 1000°/s】。
     ⚠️ 后果：偏航积分一旦削顶就【少算】—— 车实际转 360°，积分可能只到 200~300°。
        这会让 `GOURD_ARC_FLIP`（同号偏航累计到 170° 触发换向）**严重滞后甚至不触发**，
        所以它是那个方案能否成立的前提。
     ★代价：灵敏度从 32.8 → 16.4 LSB/(°/s)（分辨率减半）。但循迹的 yaw 反馈只用到
       相对变化与积分，分辨率减半可以接受；而【削顶】会让积分彻底失真 —— 两害相权取其轻。
     ★换算一致性有保障：下面那段"读回 FS_SEL 选 gyro_lsb"会按实际生效档位赋值，
       所以即使写入没生效也不会算错。 */
  imu_write_reg(0x1B, 0x18);
  HAL_Delay(10);
  if (((imu_read_reg(0x1B) >> 3) & 0x03u) != 3u)
  {
