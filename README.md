...
- 固件 **FW:0923-1917**：★**电池换成 12V 锂电池**，速度与增益已按 ×0.62 重标定
  （`BASE_SPEED 37` / 直道 43 / 弯道 30 / `KP 2.0`）—— 推导写在 `app_config.h` §二，改之前先读
- 已实现：串口在线调参 / 编码器测速（7.4V 时 `RPM ≈ 3.59 × PWM`，12V 后约 5.8 × PWM）/ 速度自适应 / 丢线兜底 / 十字形状判据 + 预算层 / **直线提速（带斜坡）** / 葫芦相切点状态机 / 最右一路计数 `G7` / MPU6050 偏航率
- 待办：终点停车（图案 + 里程双条件）、速度闭环、蜂鸣器硬件修复、**串口 RX 待实测确认**、`G7_TURN_TRIG` 待标定
- 更多细节见 `文档/给接手AI的复查提醒-20260923.md`、`文档/代码交接说明-20260921.md`、`文档/文档索引与状态.md`
